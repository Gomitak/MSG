...
// build the map
var mapId = "mapbox.streets"; // dark map
L.mapbox.accessToken = 'pk.eyJ1IjoiYmNhbzYiLCJhIjoiY2phazdzdnZjMmZwaDJ3cGRhbHYzbDJlbyJ9.W6eoGsJU4BbJoz4yglvRcw';
var map = L.mapbox.map("mapDiv", mapId).setView([31, -101], 4);
...
function pointRadius(feature) {
  return Math.max(feature.properties.Dead*0.7, 4);
}

function scaledPoint(feature, latlng) {
  // generate html for popup
  var html = "<div class='marker-title'>" + feature.properties.Summary + "</div>";
...
// get the data
d3.json("shootings3.GeoJSON", function(error, data) {
...
console.log(dateExtent);
  ...
  chart = barChart()
    ...
    .x(d3.time.scale()
      .domain(dateExtent)
      .rangeRound([-39, 4.6 * countGroup.all().length + 1]))
    .filter([new Date(1966, 8, 1), new Date(2017, 11, 12)])
  ...
  function renderAll() {
    ...
    var html = "";
    html    += "<table>";
    html    += "  <tr><td># of Weeks</td><td class='data'>" + weeks + "</td></tr>";
    ...
    if (weeks != 1) {
      html  += "  <tr class='dead'><td>Dead/Week</td><td class='data'>" + fmt1dec(avgDead) + "</td></tr>";
      html  += "  <tr class='injured'><td>Injured/Week</td><td class='data'>" + fmt1dec(avgInjured) + "</td></tr>";
    }
    html    += "</table>"
  ...
  function main(_) {
    ...
    function startStop() {
      ...
      if (value == "start") {
        ...
        d3.select("#anim_startStop").style("width", "130px")
        elem.text("Not Implemented").attr("value", "start")
